<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TopazSignatureControl.ascx.cs" Inherits="TopazSignatureControl.Controls.TopazSignatureControl" %>

<div>

    <%-- Campo oculto para la firma en B64 --%>
    <asp:HiddenField ID="SignatureHf" runat="server" />

    <%-- Campo oculto para el PadMode --%>
    <asp:HiddenField ID="PadModeHf" runat="server" />
...
<!-- Funcionalida de firma con cursor y touch -->
<script type="text/javascript">

    function CursorSign() {

        console.log("Cursor Sign");
        ...
        function stopDrawing() {
            ...
            document.getElementById('<%= SignatureHf.ClientID %>').value = base64Data;
        }

        function startDrawingTouch(e) {
            e.preventDefault();
            var touch = e.touches[0];
            var simulatedMouseEvent = new MouseEvent("mousedown", {
        ...
        function drawTouch(e) {
            e.preventDefault();
            var touch = e.touches[0];
            var simulatedMouseEvent = new MouseEvent("mousemove", {
                clientX: touch.clientX,
                clientY: touch.clientY
            });
            canvas.dispatchEvent(simulatedMouseEvent);
    ...
    function cursor_clear() {
        ...
        document.getElementById('<%= SignatureHf.ClientID %>').value = '';
    }

    // Funcion universal para comenzar la firma
    function startSignature() {
        var padMode = <%= PadModeHf.Value %>;
    ...
    // Funcion universal para borrar.
    function ClearSignature() {
        console.log("Cleared Canvas");
        var pad_mode_hf = document.getElementById('PadModeHf')
        if (pad_mode_hf.value == "1") {
            ClearTablet();
        } else {
            cursor_clear();
        }
 ...
 <!-- Funcionalida de firma con SigWeb -->
 <script type="text/javascript">
     function onSign() {
         var pad_mode_hf = document.getElementById('PadModeHf')
         pad_mode_hf.value = "1";
         console.log("Sign with tablet");
         var canvas = document.getElementById('SignatureCanvas');
         var ctx = canvas.getContext('2d');
         if (!ctx) {
             console.log('Error: Canvas not found or context not supported.');
         }
         SetDisplayXSize(400);
         SetDisplayYSize(250);
     ...
     // Funcion universal para aceptar la firma y mostrar el overlay.
     function signatureDone() {
         console.log("Done");
         var pad_mode_hf = document.getElementById('pad_mode_hf')
     ...
     // Solo llamar a esta funcion para terminar con 
     // firma de sigweb.
     function on_done_sigweb() {
         console.log("Done");
         SetTabletState(0, tmr);
         SetImageXSize(240);
         SetImageYSize(160);
     ...
     function SigImageCallback(str) {
         console.log("SigImageCallback");
         document.getElementById('<%= SignatureHf.ClientID %>').value = str;
     }
